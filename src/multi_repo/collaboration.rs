//! Team collaboration features for multi-repository diagnostics
...
impl TeamDatabase {
    ...
    pub async fn create_assignment(&self, assignment: DiagnosticAssignment) -> Result<()> {
        ...
        let status = Self::status_to_string(&assignment.status);
        let priority = Self::priority_to_string(&assignment.priority);
    ...
    pub async fn update_assignment_status(
        ...
        let new_status_str = Self::status_to_string(&new_status);
    ...
    pub async fn get_member_assignments(
        ...
        let assignments = if let Some(ref status) = status_str {
            stmt.query_map(params![member_id, status], |row| {
                Ok(DiagnosticAssignment {
                    ...
                    due_date: row
                        .get::<_, Option<i64>>(7)?
                        .map(|ts| DateTime::from_timestamp(ts, 0).unwrap()),
                    status: Self::string_to_status(&row.get::<_, String>(8)?),
                    priority: Self::string_to_priority(&row.get::<_, String>(9)?),
                    notes: row.get(10)?,
                })
            })?
            .collect::<rusqlite::Result<Vec<_>>>()?
        } else {
            stmt.query_map(params![member_id], |row| {
                Ok(DiagnosticAssignment {
                    id: row.get(0)?,
                    repository_id: row.get(1)?,
                    file_path: row.get(2)?,
    ...
    /// Get team metrics
    pub async fn get_team_metrics(&self) -> Result<Vec<(TeamMember, u32, Option<i64>)>> {
        let conn = self.conn.lock().await;
